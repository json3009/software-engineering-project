...
public class Frame : BindableElement, INotifyValueChanged<bool>
{
	...
	public const string Stylesheet = "Frame";
	public const string UssClassName = "frame";
	...
	public static readonly string ExpandIcon = "IN foldout on";
	public static readonly string CollapseIcon = "IN foldout";
	...
	private void BuildUi()
	{
		...
		this.AddUss(Stylesheet);

		Header = new VisualElement();
		Header.AddToClassList(HeaderUssClassName);
		hierarchy.Add(Header);

		_collapseButton = new Button(() => IsCollapsed = !IsCollapsed) { tooltip = CollapseTooltip };
	...
	private void UpdateCollapse()
	{
		...
		_collapseButton.style.display = (_isCollapsable) ? DisplayStyle.Flex : DisplayStyle.None;

		_collapseButton.style.backgroundImage = _isCollapsed? 
			new StyleBackground((Texture2D)EditorGUIUtility.IconContent(CollapseIcon).image):
			new StyleBackground((Texture2D)EditorGUIUtility.IconContent(ExpandIcon).image);
